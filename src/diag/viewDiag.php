...
?>
<div class="container">
    <?= $jumbo->getJumbo("Xem bệnh nhân", "Xem chuẩn đoán về bệnh nhân") ?>
        ...
        <tbody>
            <?php while ($row = $patientsList->fetch(PDO::FETCH_ASSOC)) {
                ...
                <tr>
                    ...
                    <td><?= $row['dor'] ?></td>
                    <td><?= $row['contact'] ?></td>
                    <td><a title="Thêm kiểm tra" href="addTests.php?id=<?= $row['user_id'] ?>" class="btn btn-sm btn-warning fa fa-plus"></a> </td>
                    <td><a title="Xem kiểm tra" href="viewTests.php?id=<?= $row['user_id'] ?>" class="btn btn-sm btn-default fas fa-eye"></a> </td>
