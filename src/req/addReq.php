...
?>
<div class="container">
    <?= $jumbo->getJumbo("Thêm yêu cầu", "Thên yêu cầu đơn thuốc") ?>
                ...
                <tbody>
                    ...
                    <?php while ($row = $drugsList->fetch(PDO::FETCH_ASSOC)) {
                        ...
                        <tr>
                            ...
                            <td><button title="Thêm vào giỏ hàng" class="delete btn btn-default btn-sm fa fa-cart-plus" onclick="addToCart('<?= $row['drug_id'] ?>' , '<?= $row['name'] ?>' , '<?= $row['expiry'] ?>', '<?= $row['presp'] ?>', '<?= $row['price'] ?>')"></button></td>
                        </tr>
                    <?php } ?>
                </tbody>
            </table>
            ...
            $.ajax({
                ...
                success: function(resp) {
                    if (resp == "success") {
                        ...
                        Swal.fire({
                            title: "Thêm vào giỏ hàng thất bại:  " + resp,
                            width: "100",
                            position: "top-right",
                            timer: 1000,
                            showConfirmButton: false,
        ...
        function createReq(array) {
            ...
            $.ajax({
                ...
                success: function(resp) {
                    if (resp == "success") {
                        Swal.fire({
                            title: "Yêu cầu được lưu thành công",
                            text: "Vui lòng chờ hệ thống lưu",
                            width: "100",
                            type: "success",
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                            ...
                            <?php foreach ($carts as $value) {
                                if (isset($value[1])) :
                                    ...
                                    <tr>
                                        ...
                                        <td><?= $value[4] ?></td>
                                        <td><?= $value[5] ?></td>
                                        <td><button title="Xóa khỏi giỏ hàng" class="delete btn btn-default btn-sm fa fa-eject" onclick="removeFromCart('<?= $value[0] ?>','<?= $value[1] ?>','<?= $value[2] ?>','<?= $value[3] ?>','<?= $value[4] ?>','<?= $value[5] ?>')"></button></td>
                                    </tr>
                                    <?php
                                    $val =  $value[3] * $value[5];
                                    $total = $total + $val;
                ...
                <div class="modal-footer">
                    ...
                    <input onclick="createReq()" name="Approve" value="Chấp nhận" class="btn btn-primary">
                </div>
            </div>
        </div>
    </div>
